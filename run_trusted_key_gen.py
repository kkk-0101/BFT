...
def trusted_key_gen(N=4, f=1, seed=None):
    ...
    if fi not in os.listdir(os.getcwd()):
        os.mkdir(os.getcwd() + '/' + fi)

    # public key of (f+1, n) thld sig
    with open(os.getcwd() + '/' + fi + '/' + 'sPK.key', 'wb') as fp:
        pickle.dump(sPK, fp)

    # public key of (n-f, n) thld sig
    with open(os.getcwd() + '/' + fi + '/' + 'sPK1.key', 'wb') as fp:
        pickle.dump(sPK1, fp)

    # public key of (f+1, n) thld enc
    with open(os.getcwd() + '/' + fi + '/' + 'ePK.key', 'wb') as fp:
        pickle.dump(ePK, fp)

    # public keys of ECDSA
    for i in range(N):
        with open(os.getcwd() + '/' + fi + '/' + 'sPK2-' + str(i) + '.key', 'wb') as fp:
            pickle.dump(sPK2s[i].format(), fp)

    # private key of (f+1, n) thld sig
    for i in range(N):
        with open(os.getcwd() + '/' + fi + '/' + 'sSK-' + str(i) + '.key', 'wb') as fp:
            pickle.dump(sSKs[i], fp)

    # private key of (n-f, n) thld sig
    for i in range(N):
        with open(os.getcwd() + '/' + fi + '/' + 'sSK1-' + str(i) + '.key', 'wb') as fp:
            pickle.dump(sSK1s[i], fp)

    # private key of (f+1, n) thld enc
    for i in range(N):
        with open(os.getcwd() + '/' + fi + '/' + 'eSK-' + str(i) + '.key', 'wb') as fp:
            pickle.dump(eSKs[i], fp)

    # private keys of ECDSA
    for i in range(N):
        with open(os.getcwd() + '/' + fi + '/' + 'sSK2-' + str(i) + '.key', 'wb') as fp:
